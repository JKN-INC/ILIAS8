...
class ilPCParagraph extends ilPageContent
{
    ...
    */
            ...
            foreach ($textnodes as $node) {
             ...
             foreach ($a_terms as $t) {
                 ...
                 while  (is_int($pos)) {
                     ...
                     $tex_bpos = ilStr::strPos(ilStr::subStr($node_val, 0, $pos), "[tex]");
     ...
     * @return
     */
    protected static function unlinkTermsInDom($a_dom, $a_terms, $a_par_node = null): void
    {

        foreach ($a_terms as $k => $t) {
            if (isset($t['alternates']) && is_array($t['alternates'])) {
                foreach ($t['alternates'] as $alt) {
                    array_push($a_terms, array(
                        "term" => $alt,
                        "language" => $t["language"],
                        "id" => $t["id"],
                        "glo_id" => $t["glo_id"],
     ...
     * @return
     */
    public static function removeLinkGlossariesPage($a_page, $a_terms): void
    {
        $a_page->buildDom();
        $a_dom = $a_page->getDom();
