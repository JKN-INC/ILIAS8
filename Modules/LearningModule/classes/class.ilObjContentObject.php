...
class ilObjContentObject extends ilObject
{
     ...
     * @param $term
     */
    public function unLinkGlossaryTerm($term): void
    {
        ...
        foreach ($pages as $p) {
            ...
            if (is_int(stripos($c, $term["term"]))) {
                ...
                if (!isset($found_pages[$p["id"]]["page"]) || !is_object($found_pages[$p["id"]]["page"])) {
     ...
     * @param $a_glo_ref_id
     */
    public function unLinkGlossaryTerms($a_glo_ref_id): void
    {
    ...
    public function autoLinkGlossaryTerms(
        ...
        foreach ($pages as $p) {
            ...
            foreach ($terms as $t) {
                if (is_int(stripos($c, $t["term"]))) {
                    $found_pages[$p["id"]]['terms'][] = $t;
                    if (!isset($found_pages[$p["id"]]['pages'][$p['lang']])) {
